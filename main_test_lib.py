...
class MambaBlock(nn.Module):
    def __init__(self, d_model, expansion_factor=2, kernel_size=4):
        super().__init__()
        self.d_model = d_model
        self.inner_dim = int(d_model * expansion_factor)
        self.kernel_size = kernel_size

        # Convolutional layer for local context mixing
        self.conv = nn.Conv1d(
            in_channels=d_model,
        ...
        # Linear projections for SSM parameters and input
        self.in_proj = nn.Linear(d_model, self.inner_dim * 4)  # delta, B, C, x
    ...
    def forward(self, x):
        ...
        conv_x = self.conv(x.transpose(1, 2)).transpose(1, 2)
        conv_x = conv_x[:, :L, :]  # remove padding.

        inner_x = self.in_proj(conv_x)
        delta, B_param, C_param, x_ssm = torch.split(inner_x, self.inner_dim, dim=-1)

        # Apply activation functions
        delta = F.softplus(delta)
        B_param = self.act(B_param)
        C_param = self.act(C_param)

        # SSM Core (simplified)
        state = torch.zeros(B, self.inner_dim, device=x.device)
        output = []
        for l in range(L):
            A = torch.exp(-delta[:, l, :].unsqueeze(-1))
            state = A * state + B_param[:, l, :].unsqueeze(-1) * x_ssm[
                :, l, :
            ].unsqueeze(-1)
            out = torch.matmul(C_param[:, l, :].unsqueeze(1), state).squeeze(1)
            output.append(out)
...
class Mamba(nn.Module):
    def __init__(self, d_model, num_layers, expansion_factor=2, kernel_size=4):
        super().__init__()
        self.layers = nn.ModuleList(
            [
                MambaBlock(d_model, expansion_factor, kernel_size)
