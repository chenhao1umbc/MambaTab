...
model = MambaTab(
    input_features=1, n_class=1
)  # Dummy model initialization. Later this is changed according to feature set length.
for incremental in range(3):
    ...
    dataloader = {
        "train": DataLoader(
            train_set, batch_size=config["BATCH"], shuffle=True, num_workers=0
        ),
        "val": DataLoader(
            val_set, batch_size=config["BATCH"], shuffle=False, num_workers=0
        ),
        "test": DataLoader(
            test_set, batch_size=config["BATCH"], shuffle=False, num_workers=0
        ),
    }
    model.linear_layer = torch.nn.Linear(
        x_train.shape[1], config["REPRESENTATION_LAYER"]
    )  # Here we only change the first layer's input shape for adapting on feature incremental learning setting
